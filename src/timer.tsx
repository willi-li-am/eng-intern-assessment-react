...
const LapseItem = ({time, index} : any) => {
    return (
        <div style={{padding: "10px"}}>
            ...
            >
                <div style={{fontSize: "20px", marginRight: '20px', width: "150px"}}>Lapse {index}:</div>
    ...
    return (
        <div style={{overflow: 'auto', width: "100%"}}>
            {reverseLapse.map((time: number, index: number) => {
                return <LapseItem key={"lapse" + index} time={time} index={reverseLapse.length - index}></LapseItem>
            })}
        </div>
    )
}
